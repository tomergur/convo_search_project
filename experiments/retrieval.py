...
from convo_search_project.rewriters import AllHistoryRewriter, PrevUtteranceRewriter, FirstUtteranceRewriter, \
    SeparateUtterancesRewriter, T5Rewriter, QuReTeCRewriter, FileRewriter, HqeRewriter
...
def parse_args():
    ...
    parser.add_argument('--hqe', action='store_true', help='use hqe rewriter')
    ...
    parser.add_argument("--second_stage_rewriter",
                        help="select a second stage rewriter that is diffrent from the first stage one")
...
def run_exp(args, pipeline):
    ...
    for session in data:
        ...
        for rewriter in rewriters:
            if isinstance(rewriter, Hqe_rewriter):
                print("reset history")
...
if __name__ == "__main__":
    ...
    if 'hqe' in args and args.hqe:
        rewriters.append(HqeRewriter(searcher))

    second_stage_rewriter = None
    if 'second_stage_rewriter' in args:
        if args.second_stage_rewriter == "T5_rewriter":
            second_stage_rewriter = t5_rewriter if t5_rewriter else T5Rewriter(model_str=args.T5_model_str)
    reranker = build_bert_reranker2(batch_size=args.reranker_batch_size) if args.rerank else None
    pipeline = Pipeline(searcher, rewriters, count, reranker, second_stage_rewriter)
